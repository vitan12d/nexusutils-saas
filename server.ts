...
async function startServer() {
  ...
  app.post("/api/ai/writing-assistant", async (req: any, res: any) => {
    try {
      const { text, action, tone, systemInstruction, customPrompt } = req.body;
      if (!text && !customPrompt) {
        return res.status(400).json({ error: "Text or customPrompt is required" });
      }

      let prompt = "";
      let sysInstr = "You are a professional AI Writing Assistant for NexusUtils. Provide high-quality proofread, summarized, expanded, or rewritten text according to user preferences. Only return the processed text directly without any extra chat context, introductions, or conversational filler.";
