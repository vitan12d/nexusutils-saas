...
export default function App() {
  ...
  const [currentRoute, setCurrentRoute] = useState<{ type: 'dashboard' | 'match' | 'news'; id?: string }>({
    type: 'dashboard',
  });
  ...
  const resolveMatch = (slugOrId: string | undefined): Match | undefined => {
    ...
    const exact = matches.find((m) => m.id === slugOrId);
    if (exact) return exact;

    // 2. Format is "id-slug", check if prefixed with "id-"
    const prefixed = matches.find((m) => slugOrId.startsWith(m.id + '-'));
    if (prefixed) return prefixed;

    // 3. Fallback: match using generateSlug directly
    const slugMatch = matches.find((m) => {
  ...
  const renderContent = () => {
    ...
    switch (currentRoute.type) {
      ...
      case 'dashboard':
      default:
        return <Dashboard matches={matches} newsArticles={newsArticles} searchQuery={searchQuery} />;
    }
  };
