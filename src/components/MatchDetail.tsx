...
export default function MatchDetail({ match }: MatchDetailProps) {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        const fixtureId = match.id.replace('api-', '');
        const res = await fetch(`/api/match-details?id=${fixtureId}`);
        if (res.ok) {
        ...
        console.error('Failed to fetch api-football details:', err);
      } finally {
  ...
  const activeMatch = detailedMatch || match;
  ...
  const league = LEAGUES[activeMatch.leagueId];
  ...
  const sportsEventSchema = {
    ...
    '@type': 'SportsEvent',
    'name': `${activeMatch.homeTeam.name} vs ${activeMatch.awayTeam.name} - ${league?.name || 'Football'} Live Score & Stats`,
    'startDate': `${activeMatch.date}T${activeMatch.displayTime}:00-07:00`,
    'location': {
      '@type': 'Place',
      'name': activeMatch.stadium,
    },
    ...
    'description': `${activeMatch.homeTeam.name} versus ${activeMatch.awayTeam.name} in the ${league?.name || 'Champions League'}. Check start match time, direct commentator, and online broadcasting channel guide on GOALSTATS Pro.`,
  };

  const currentHomeScore = match.status === 'LIVE' ? tickerScore.home : activeMatch.homeScore;
  const currentAwayScore = match.status === 'LIVE' ? tickerScore.away : activeMatch.awayScore;
  const currentMinute = match.status === 'LIVE' ? tickerScore.min : activeMatch.minute;
      ...
      <div className="bg-[#16191E] border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-center bg-gradient-to-b from-[#16191E] to-[#0F1115]">
          ...
          <div className="flex flex-col items-center md:items-end text-center md:text-right space-y-3">
            {activeMatch.homeTeam.logo.startsWith('http') || activeMatch.homeTeam.logo.includes('/') ? (
              <img
                referrerPolicy="no-referrer"
                src={activeMatch.homeTeam.logo}
                alt={activeMatch.homeTeam.name}
                className="w-16 h-16 object-contain rounded-full shadow-lg border-2 border-white/10 bg-slate-900/60 p-2"
              />
            ) : (
              <div className={`w-16 h-16 ${activeMatch.homeTeam.logo} rounded-full flex items-center justify-center shadow-lg border-2 border-white/10 text-white font-black text-xl tracking-tight`}>
                {activeMatch.homeTeam.code}
              </div>
            )}
            <div>
              <h2 className="text-xl md:text-2xl font-extrabold text-slate-100">{activeMatch.homeTeam.name}</h2>
          ...
          <div className="flex flex-col items-center justify-center text-center space-y-3">
            {match.status !== 'UPCOMING' ? (
              ...
              <div className="text-xs font-mono font-bold bg-slate-950 text-slate-400 border border-slate-800 px-4 py-2 rounded-xl">
                MATCH TIMING : <span className="text-emerald-400 text-sm font-black ml-1">{activeMatch.displayTime}</span>
              </div>
            )}
            ...
            <div className="space-y-1">
              <p className="text-xs font-mono text-slate-400 flex items-center gap-1.5 justify-center">
                <Calendar className="w-3.5 h-3.5 text-emerald-400" /> Match Date: {activeMatch.date}
              </p>
              <p className="text-xs font-mono text-slate-400 flex items-center gap-1.5 justify-center">
                <MapPin className="w-3.5 h-3.5 text-emerald-400" /> {activeMatch.stadium}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left space-y-3">
            {activeMatch.awayTeam.logo.startsWith('http') || activeMatch.awayTeam.logo.includes('/') ? (
              <img
                referrerPolicy="no-referrer"
              ...
              <div className={`w-16 h-16 ${activeMatch.awayTeam.logo} rounded-full flex items-center justify-center shadow-lg border-2 border-white/10 text-white font-black text-xl tracking-tight`}>
                {activeMatch.awayTeam.code}
              </div>
            )}
            <div>
              <h2 className="text-xl md:text-2xl font-extrabold text-slate-100">{activeMatch.awayTeam.name}</h2>
        ...
        <div className="bg-slate-950 p-4 border-t border-slate-850/80 grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs font-mono text-slate-400 text-center sm:text-left">
          <div className="flex items-center gap-2 justify-center sm:justify-start">
            ...
            <span>Referee: <strong className="text-slate-300">{activeMatch.referee}</strong></span>
          </div>
          <div className="flex items-center gap-2 justify-center sm:justify-start">
            <Tv className="w-4 h-4 text-emerald-400 shrink-0" />
            <span>TV Network: <strong className="text-slate-300">{activeMatch.tvChannel}</strong></span>
          </div>
          <div className="flex items-center gap-2 justify-center sm:justify-start">
            <Award className="w-4 h-4 text-emerald-400 shrink-0" />
            <span>Microphone Sport Narrator: <strong className="text-slate-300">{activeMatch.commentator}</strong></span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#1A1D23] border border-slate-800 rounded-2xl overflow-hidden shadow-lg p-5 sm:p-6 text-left">
          ...
          <div className="space-y-6" id="panel-info">
            ...
            <article className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-extrabold text-slate-100">{activeMatch.reportTitle}</h3>
              </div>
              ...
              <div className="text-slate-300 text-xs sm:text-sm leading-relaxed space-y-4">
                <p>
                  Welcome to GOALSTATS pro coverage detailing the grand sports confrontation scheduling for <strong className="text-emerald-400 underline">{activeMatch.homeTeam.name}</strong> vs <strong className="text-emerald-400 underline">{activeMatch.awayTeam.name}</strong>, taking place at the esteemed {activeMatch.stadium}. Football fans who need to know <strong className="text-slate-100">how to watch the game live online</strong> will find a complete listing of verified domestic and international satellite TV networks below, alongside details of scheduled commentators and match times.
                </p>
                <p>
              ...
              <div className="mt-8 pt-6 border-t border-slate-850 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {activeMatch.headToHead.map((h, idx) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 border-b border-white/10 pb-2">
                    ...
                    <h4 className="font-extrabold text-sm text-slate-100">{activeMatch.homeTeam.name} Lineup</h4>
                  </div>
                  <div className="space-y-2">
                    {activeMatch.lineups.home.map((p, pidx) => (
                      <div key={`${p.number}-${pidx}`} className="bg-slate-950/80 hover:bg-slate-950 border border-emerald-500/10 px-3 py-2 rounded-lg flex items-center justify-between text-xs">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 border-b border-white/10 pb-2">
                    ...
                    <h4 className="font-extrabold text-sm text-slate-100">{activeMatch.awayTeam.name} Lineup</h4>
                  </div>
                  <div className="space-y-2">
                    {activeMatch.lineups.away.map((p, pidx) => (
                      <div key={`${p.number}-${pidx}`} className="bg-slate-950/80 hover:bg-slate-950 border border-emerald-500/10 px-3 py-2 rounded-lg flex items-center justify-between text-xs">
                        <div className="flex items-center gap-3">
                          <span className="font-mono text-emerald-400 font-bold shrink-0 w-5 text-right">{p.number}</span>
                          <span className="font-medium text-slate-200">{p.name}</span>
        ...
        {activeTab === 'stats' && (
          <div className="space-y-6" id="panel-stats">
            ...
            {isDetailLoading ? (
              <div className="flex flex-col items-center justify-center py-12 text-emerald-400 font-mono text-xs gap-3">
                <RefreshCw className="w-6 h-6 animate-spin text-emerald-400" />
                <span>Processing tactical statistics...</span>
              </div>
            ) : (
              <div className="space-y-5">
                {activeMatch.stats.map((s, idx) => (
