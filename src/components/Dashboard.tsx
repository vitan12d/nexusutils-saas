...
function ToolsHubOverview({
  ...
  const toolsToShow = TOOLS.filter(t => {
...
export default function Dashboard({ initialCategoryId, onGoHome, onGoStaticPage }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0F172A] text-slate-800 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200">
      {/* 1. Header Navigation */}
      <header className="sticky top-4 mx-4 my-2 bg-white/80 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/5 rounded-2xl z-30 shadow-none">
        <div className="max-w-7xl mx-auto px-4 h-15 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-1.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg lg:hidden transition"
            >
          ...
          <div className="hidden sm:flex items-center gap-2 flex-1 max-w-md relative">
            ...
            <input
              ...
              placeholder="Instant search across 20+ operations..."
          ...
          <div className="flex items-center gap-3">
            {/* Live System Banner */}
            <div className="hidden sm:flex items-center gap-1.5 text-[10px] font-bold bg-emerald-500/10 dark:bg-emerald-950/20 border border-emerald-500/25 px-3 py-1.5 rounded-full text-emerald-600 dark:text-emerald-400 select-none">
            ...
            <button
              onClick={() => onGoStaticPage('contact')}
              className="hidden md:block py-1.5 px-3.5 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-lg font-bold text-xs text-slate-705 dark:text-slate-250 hover:bg-slate-50 transition cursor-pointer"
            >
              Contact support
            </button>
          </div>
      ...
      <div className="flex-1 flex max-w-7xl mx-auto w-full relative">
        {/* Sticky left panel panel */}
        <aside
          className={`fixed inset-y-0 left-0 lg:static w-64 border-r border-slate-200/60 dark:border-white/5 bg-white dark:bg-slate-900 lg:bg-transparent lg:dark:bg-transparent z-40 lg:z-10 transition-transform duration-300 ${
            sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
          }`}
        >
          <div className="p-4 lg:sticky lg:top-24 space-y-5 h-[calc(100vh-5.5rem)] overflow-y-auto flex flex-col justify-between">
            <div className="space-y-5">
              ...
              <div className="sm:hidden relative">
                ...
                <input
                  ...
                  className="w-full py-2 pl-9 pr-4 bg-slate-50 dark:bg-slate-950/20 border border-slate-200 dark:border-slate-800 rounded-lg text-xs outline-none focus:border-blue-500 text-slate-100"
                />
              </div>

              {/* Categories */}
              <div className="space-y-1.5">
                <span className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-3 block mb-1.5">
                  SUITES Categories
                </span>
                <button
                  onClick={() => {
                  ...
                  className={`w-full py-2 px-3.5 text-left font-bold text-xs flex items-center gap-2.5 rounded-xl transition ${
                    activeCategory === 'all'
                      ? 'bg-blue-50 dark:bg-blue-955/40 text-blue-700 dark:text-blue-400 border border-blue-500/10'
                      : 'text-slate-550 dark:text-slate-400 hover:bg-slate-100/50 dark:hover:bg-slate-800/10 border border-transparent'
                  }`}
                >
                ...
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full py-2 px-3.5 text-left font-bold text-xs flex items-center justify-between rounded-xl transition ${
                        activeCategory === cat.id
                          ? 'bg-blue-50 dark:bg-blue-955/40 text-blue-700 dark:text-blue-405 border border-blue-500/10'
                          : 'text-slate-550 dark:text-slate-400 hover:bg-slate-100/50 dark:hover:bg-slate-800/10 border border-transparent'
                      }`}
                    >
                ...
                <div className="space-y-1 max-h-[220px] overflow-y-auto pr-1">
                  {filteredTools.map((tool) => (
                    <button
                      ...
                      className={`w-full py-1.5 px-3 text-left text-xs font-semibold rounded-lg flex items-center justify-between transition ${
                        activeTool?.id === tool.id
                          ? 'bg-slate-100 dark:bg-slate-800/60 text-blue-600 dark:text-blue-400 font-bold'
                          : 'text-slate-650 dark:text-slate-355 hover:bg-slate-100/50 dark:hover:bg-slate-900/10'
                      }`}
                    >
                      <span className="truncate">{tool.name}</span>
                      <ChevronRight className={`h-3 w-3 ${activeTool?.id === tool.id ? 'opacity-100' : 'opacity-0'}`} />
                    </button>
                  ))}
                  {filteredTools.length === 0 && (
                    <span className="text-[10px] text-slate-400 pl-3 block py-2">No matching products found.</span>
                  )}
                </div>
              </div>
            </div>

            {/* Retro Bento Upgrade Now promo */}
            <div className="pt-4 border-t border-slate-100 dark:border-white/5 space-y-4">
              <div className="p-3.5 bg-indigo-900/5 dark:bg-indigo-950/20 border border-indigo-500/15 dark:border-indigo-500/10 rounded-xl">
                <p className="text-[10px] text-[#2563EB] dark:text-indigo-400 font-extrabold uppercase tracking-widest mb-1">Nexus Premium</p>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-normal mb-2.5">Instant 500MB file limits and lightning batch operations.</p>
                <button className="w-full py-1.5 bg-indigo-650 hover:bg-indigo-600 rounded-lg text-[10px] font-bold text-white transition cursor-pointer shadow-xs">Upgrade Now</button>
              </div>
        ...
        <main className="flex-1 min-w-0 p-4 sm:p-6 lg:p-8 space-y-8">
          {activeTool ? (
            <div className="space-y-8 animate-fade-in">
              ...
              {/* Dynamic Top AdSense Premium Zone Banner */}
              ...
              <div className="p-5 sm:p-7 bg-white dark:bg-slate-900/60 rounded-2xl border border-slate-200/80 dark:border-white/5 relative min-h-[400px] shadow-xs overflow-hidden group">
                ...
                {activeTool.category === 'ai' && <AiTools />}
              </div>

              {/* Secure Bottom Anchor AdSense Placeholder */}
